...
Descreva as ações esperadas pelos alunos na cena. Descreva as ações esperadas pelos atores e pelo simulador se houver. Se houver mais de um ator, especifique quais ações cada um deve realizar por cena, não deixe que um ator não interaja com os alunos durante a cena. Caso haja a participação de um simulador descreva também como o simulador interage com os alunos e atores e a evolução de seus parâmetros durante a cena. Em student_plan_b descreva as ações esperadas pelos alunos caso as ações desejadas não sejam executadas.
